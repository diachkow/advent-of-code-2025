...
export default class extends Puzzle {
  override readonly expectedTestResult = 40;

  override async solution(inputPath: string): Promise<number> {
    const [MAX_CONNECTIONS, junctionBoxes] = await this.processInput(inputPath);
    const distances: BoxDistance[] = this.calculateDistances(junctionBoxes);
    distances.sort((a, b) => a.distance - b.distance);
    const circuits = this.mergeBoxesIntoCircuits(junctionBoxes, distances, MAX_CONNECTIONS);
    return this.estimateExtensionCableSizes(circuits);
  }
  ...
  private mergeBoxesIntoCircuits(
    ...
    const uniqueCircuits: Circuit[] = [];
    for (const box of junctionBoxes) {
      if (box.circuit !== null && !uniqueCircuits.includes(box.circuit)) {
  ...
  private estimateExtensionCableSizes(circuits: Circuit[]): number {
    const topThree = circuits.toSorted((a, b) => b.size() - a.size()).slice(0, 3);
    return topThree.reduce((acc, val) => acc * val.size(), 1);
  }
}
